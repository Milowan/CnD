...
class Fog : Entity
{
    ...
    override func collision(response: Entity)
    {
        ...
        if response.collisionMask == .PLAYER
        {
            ...
            bBottom = response.pos.y - Int(response.sprite!.size.height / 2)
            bTop = response.pos.y + Int(response.sprite!.size.height / 2)
            bLeft = response.pos.x - Int(response.sprite!.size.width / 2)
            bRight = response.pos.x + Int(response.sprite!.size.width / 2)
